        ...
        foreach my $bite (@sequence) {
                ...
                foreach my $note ($bite->notes) {
                        ...
                        if ($type eq 'loopbegin') {
                                ...
                                if ($to < $from) {
                                        $worksheet->appendCode("FOR ScrW0 = $to TO $from" . ($step > 1 ? " STEP $step" : ''));
                                        $worksheet->indentCode;
                                        $loopindex = '(' . ($from + $to) . " - ScrW0)";
                                } else {
                                        $worksheet->appendCode("FOR ScrW0 = $from TO $to" . ($step > 1 ? " STEP $step" : ''));
                                        $worksheet->indentCode;
                                        $loopindex = "(0 + ScrW0)"
                                }
                                ...
                                $worksheet->appendCode(uc($type) . " ($loopindex = $condition) THEN");
                                $worksheet->indentCode
                                ...
                                if ($type eq 'pulse') {
                                                ...
                                                $worksheet->appendCode("PULSOUT Speaker, $freq * 5");
                                                $worksheet->appendCode("PAUSE $duration") if $duration

                                } elsif ($freq) {

                                        $worksheet->appendCode("FREQOUT Speaker, $duration, $freq * 5") if $duration
...
sub _createEditWindow {
        ...
        my $tilecolor = '#00E700';
        my $selectcolor = 'CYAN';
        my $mw = $scribbler->mainwindow->Toplevel(-background => $BG, -title => ucfirst($scribbler->translate('create sounds')));
        $mw->withdraw;
