...
sub emitCode {
        ...
        $timer = int($timer * 40 + 0.5);
        my $temp = $TrimIndex + 127;
        if ($type eq 'set') {
                $left += 100; $right += 100;
                $worksheet->appendCode("LeftMotor = $left: RightMotor = $right: MoveTime = $timer: Trim = $temp");
                $worksheet->emitCall('MotorSet');
        } elsif ($type eq 'add') {
                $left += 100; $right += 100;
                $worksheet->appendCode("MoveLeft = $left: MoveRight = $right: MoveTime = 0");
                $worksheet->emitCall('MotorAdd')
        } elsif ($type eq 'mul') {
                $left = int($left * 64 + 100.5);
                $right = int($right * 64 + 100.5);
                $worksheet->appendCode("MoveLeft = $left: MoveRight = $right: MoveTime = 0");
                $worksheet->emitCall('MotorMul')
        }
}
...
sub _createEditWindow {
        ...
        my $mw = $scribbler->mainwindow->Toplevel(-title => ucfirst($scribbler->translate('motion')), -background => $BG);
        ...
        $TrimLabel = $frame2->Label(-text => 'Trim: ' . $TrimIndex, -foreground => 'white', -background => $DARK_SCRIBBLER_BLUE,
                        -font => $scribbler->font('tilefont'), -width => 8)->pack(-side => 'left', -anchor =>
                        'w');
        my $scaTurn = $frame2->Scale(
                ...
                -to =>127,
                -cursor => 'hand2',
                -command => \&_evtTrimSlide,
                -variable => \$TrimIndex
        )->pack(-side => 'left', -anchor => 'w');
        $scribbler->tooltip($scaTurn, 'Trim amount');
        ...
        my $scaTime = $frame2->Scale(
                ...
                -variable => \$TimeIndex
        )->pack(-side => 'right', -anchor => 'e', -expand => 0, -fill => 'x');
