# 12/17/10 Steve: Added the basic framework for chosing which preamble to use based on the configuration settings
# 12/21/2010 Steve: Added preamble choice functionality using user choice.  Changed error message when failed upload occurs.
# 12/30/2010 DONE: Added emitCalibrate feature that will open the calibration program in basic stamp editor
                  #Changed the starting directory of any prompt to be in the current working directory
                  #Changed extension of save files from scb to bbf (botbrain file)
# 1/6/2011 TODO: Finish making it so that only 1 configuration tile can be loaded at a time
# 1/17/2011 Done: Finished Configuration code: TODO: Reset configuration count at right times
#           Done: Fixed savefile / loadfile bug in which a light conditional would not save
...
use Scribbler::SetupTile;
require Scribbler::StandardTile;
...
sub emitCode {
        ...
        $self->{indent} = 0;
        $self->scribbler->openInit;
        my $pinc = $self->scribbler->getPinConfiguration;
        my $preamble = "Preamble".$pinc;
        print $preamble;
        ...
        if (open(BS2, ">$TEMP_DIR/temp.bs2")) {
                print BS2 $self->{code};
                close BS2;
                $self->tokenizeFile("$TEMP_DIR/temp.bs2", $action);
...
sub emitCalibrate {
    ...
    my $scribbler = $self -> scribbler;
    my $dir = getcwd;
    if (open (BS2, "$dir\\calibrate.bs2")) {
        close BS2;
        $self->tokenizeFile("$dir\\calibrate.bs2", $action);
    } else {
        print "Calibrate not uploaded";
    }
}
...
sub saveFile {
        ...
        my $defaultfile = $self->{filename} || $scribbler->init('lastfile') || "$INIT_DIR/Data/" . $self->{name} . ".bbf";
        my $defaultdir = File::Spec->canonpath(dirname($defaultfile));
        my $defaultname = basename($defaultfile);
        my $dir = getcwd . "/My Programs/";
        ...
        while (
                my $savefile = $mw->getSaveFile(
                        ...
                        -defaultextension => '.bbf',
                        -filetypes => [['BotBrain Files' => '.bbf'], ['All Files' => '.*']]
                )
        ) {
                if (open SCB, ">$savefile") {
                        my $code = XMLout($self->encode2);
                        my $digest = uc md5_hex(encode_utf8($code));
...
sub loadFile {
        ...
        my $scribbler = $self->scribbler;
        my $mw = $scribbler->mainwindow;
        my $defaultfile = $scribbler->init('lastfile') || "$INIT_DIR/Data/" . $self->{name} . ".bbf";
        my $defaultdir = File::Spec->canonpath(dirname($defaultfile));
        my $defaultname = basename($defaultfile);
        my $dir = getcwd . "/My Programs/";
        my $cdir = File::Spec -> canonpath ( $dir );
        while (
                ...
                if (open SCB, "<$loadfile") {
                        <SCB>;
                        $scribbler->resetConfigurationCount;
                        my $digest = (<SCB> =~ m/<!--\s*([0-9A-F]+)\s*-->/)[0];
                        local $/ = undef;
                        my $code = <SCB>;
                        my $conf = substr(substr($code, rindex($code, "pinconfiguration="), 19), 18);
                       	$scribbler->setPinConfiguration($conf);
                       	print "config ", $scribbler->getPinConfiguration, "\n";
                        if ($digest && $digest eq uc md5_hex($code)) {
                                $self = $self->_newFile;
                                my $code = XMLin($code, ForceArray => qr/^_/);
                                $code = recode($code);
                                #Hack here with decode2, found in Atom.pm
                                $self->decode2($code);
                                ...
                                return if $scribbler->dialog(-text => 'Chosen file is not a valid BotBrain file. Please pick another.', -buttons => ['okay', 'cancel']) eq 'cancel'
                        }
                } else {
...
sub newFile {
        ...
        $self->activateRoot;
        $scribbler->resetConfigurationCount;
        $scribbler->setPinConfiguration(1);
        $self->dirty(0);
        return $self;
}
...
sub encode2 {
        ...
        return {
                ...
                pinconfiguration => $self->scribbler->getPinConfiguration,
...
#This is the code for uploading a file
sub tokenizeFile {
        my $self = shift;
        my $scribbler = $self->scribbler;
        my $file = '"' . File::Spec->canonpath(shift()) . '"';
        my $action = shift;
        if (my $editor = $self->scribbler->editor) {
                #print "$editor\n";
                if ($action eq 'run') {
                #/noprompts is needed to not bring up a dialog box
                        my $status = `"$editor" /nodebug /noprompts /download $file`;
                        #print qq("$editor" /nodebug /noprompts /download $file\n);
                        system qq("$editor" /nodebug /noprompts /download $file);
                        #print "$file\n";
                        my $status = $scribbler->getrun(qq("$editor" /nodebug /noprompts /download $file), "Uploading...");
                        #print $status;
                        if (lc($status) =~ m/err:/s) {
                                if ($status =~ m/eeprom full|data occupies/i) {
                                        $scribbler->dialog(-text => "The program is too big for BotBrains's memory. Please shorten it and try again.", -button => 'Okay')
                                } elsif ($status =~ m/no basic stamp/i) {
                                        $scribbler->dialog(-text => [
                                                'No connection could be made to your BotBrain.', "\n\n",
                                                'Please verify the following:',"\n\n",
                                                '1. ', 'The data cable is plugged in at both ends.', "\n",
                                                '2. ', 'Your BotBrain is turned on.', "\n",
                                                '3. ', 'You have fresh batteries installed.'
                                        ],
                                        -justify => 'left')
                                } else {
                                        $status =~ m/err:\d\d\d-([^\.]+\.)/is;
                                        $scribbler->dialog(-text => ['An error occurred while uploading your program.',' You may be using sensors not available in the current setup. Please check your setup and try again.'], -button => 'Okay')
                                }
                                $status =~ s/\s*[\r\n]+\s*/;/gs;
...
# Figure out a way to make the result of string contain the tile name, not just 1
sub deleteSelected {
        my $self = shift;
        my $scribbler = $self->scribbler;
        my $string = $self->{selected}[0];
        print "\nString: ", $string, "\n";
        my $result = rindex($string, "SetupTile");
        if($result > -1)
        {
          $scribbler->resetConfigurationCount;
        }
        print "Result of search is: ", $result, "\n";
