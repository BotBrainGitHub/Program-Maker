# 12/09/2010
# Removed observation and computation tiles
#12/13/10 Joe: reordered buttons on left of worksheet
#12/17/10 Steve: added a setup tile
#12/21/10 Steve: TODO: Make SetupTile work in the top menu and not on left, added LED tiles back in
#12/30/10 DONE: Code added to hide black screen (flashes but not long)
                #Calibration added back in using emitCalibrate
#1/6/11 DONE: Added code that will limit the number of configuration tiles on the worksheet

use strict;
#use lib '/jobs/perl/scribbler'; #Must comment out for perl2exe.
#use lib '/perl/pgmmkr_2_0Beta'; #must be the name of the folder
use lib 'C:/Perl/pgmmkr_2_2';
use Carp;
use Tk;
use Tk::ToggleButton;
...
my $MenuCommands = Tie::IxHash->new(
        ...
        run => {
                ...
                tip => 'Copy the program to the BotBrain.'
        },

        edit => {
                icon => 'edit',
                onselect => sub {$Scribbler->worksheet->emitCode('edit')},
                tip => 'View the BASIC Stamp program.'
        },

#        restore => {
#                icon => 'immaculate',
#                onselect => sub {$Scribbler->worksheet->tokenizeFile("$INIT_DIR/default.bs2", 'run')},
...
my $ObjectButtons = Tie::IxHash->new(
    ...
    #  12/09/2010 taken out; not needed
    #    query => {
    #            icon => 'binocs',
    ...
    #    },
        ...
        config=> {
                icon => 'setup',
                onclick=> sub{insertNewSetup()},
                tip => 'Setup the robot'
        },

        standard => {
                icon => 'servo',
                onclick => sub{InsertNew('StandardTile')},
                tip => 'Standard Servos'
        }
);
...
#Hack to ensure that only 1 setup tile is in place
sub insertNewSetup {
    my $count = $Scribbler->getConfigurationCount();
    #print "count is ", $count , "\n";
    if($count == 0)
    {
     # $Scribbler->incrementConfigurationCount();
      InsertNew('SetupTile');
    }
}