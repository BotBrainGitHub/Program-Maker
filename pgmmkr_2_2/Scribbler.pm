# 12/21/2010
# Steve: DONE: Added $pinconfig variable, pinconfig will be 1 by default and can be changed by
# the setup option or by loading a saved file that had a changed pin configuration
#1/17/2011 DONE: Added configuration subs into this file
...
#Ensures that only 1 configuration tile is placed
my $changeConfigurationCount = 0;
sub incrementConfigurationCount {
    $changeConfigurationCount = $changeConfigurationCount + 1;
}
sub getConfigurationCount {
    return $changeConfigurationCount;
}
sub resetConfigurationCount {
    $changeConfigurationCount = 0;
}
...
sub new {
        ...
        $self->{tooltips} = 1;
        $self->{size} = [0, 0];
        if (open(CODE, "<$INIT_DIR/include.bs2")) {
                my $section;
                while (<CODE>) {
                        if (m/'%%(.+):(.*)/) {
                                my $key = lc $1;
        ...
        #Could change the -explore to be the folder of the program
        my $stampw = $self->init('editor') || '';
        my $cdir =  File::Spec->canonpath ( getcwd . "\\Editor\\" );
        until ((lc $stampw) =~ m/stampw.exe/ && (-e $stampw)) {
                $stampw = $self->dialog(
                        -title => 'BotBrain',
                        -text => 'Please help me find the BASIC Stamp editor, "stampw.exe"',
                        -explore => $cdir,
                        -buttons => ['okay', 'cancel']
                );
       ...
       # $pinconfig = $self->getPinConfiguration;
       # print "pin config ", $pinconfig, "\n";
        $self->saveInit;
        return $self
}
sub getPinConfiguration {
    my $self = shift;
    #print "getpinconfig ", $pinconfig, "\n";
    return $pinconfig
}
sub setPinConfiguration {
    my $self = shift;
    my $config = shift;
    $pinconfig = $config;
}
sub openInit {
       my $self = shift;
       if (open(INI, "<$INIT_DIR/BotBrainSettings.ini")) {
          while (<INI>)
          {
                s/\n\r//g;
...
sub retitle {
        my $self = shift;
        my $title = shift;
        $self->mainwindow->configure(-title => $self->translate('BotBrain Program Maker') . " v$VERSION     $title")
}
...
sub translate {
        ...
        if (my $foreign = $self->{'translate'}->{$key} ) {
                $foreign = ucfirst $foreign if $english eq ucfirst $english;
                if ($foreign !~ m/[\.?!]$/) {
...
sub toolWarning {
        ...
        unless ($warning = $self->{toolwarning}->{$mw}) {
                $warning = $self->{toolwarning}->{$mw} = $mw->Balloon(-state => 'balloon', -balloonposition => 'mouse', -initwait => 100, -background => 'RED', -foreground => 'WHITE');
        }
        if (ref $text) {
...
sub tooltip {
        ...
        unless ($tip = $self->{tooltip}->{$mw}) {
                $tip = $self->{tooltip}->{$mw} = $mw->Balloon(-state => 'balloon', -balloonposition => 'mouse', -initwait => 600, -background => '#FFFF80');
        }
        if (ref $text) {
                $text = {map {$_ => $self->wrap($self->translate($text->{$_}))} keys %$text};
...
sub dialog {
        ...
        my $dialog = $mw->Toplevel(-title => $feature{-title} || 'BotBrain');
